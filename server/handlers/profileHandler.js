...
router.post("/faculty/:id/", async (req, res) => {
  try {
    ...
    res.status(200).send({ user, certifications });
  } catch (error) {
    logger.error({
      code: "STU-DSH-100",
      message: "Error fetching dashboard data",
...
router.post("/faculty/:id/updatepfp", verifyToken, async (req, res) => {
  const image = req.body;
  const { mid } = req.user;
...
router.post("/faculty/:id/update", verifyToken, async (req, res) => {
  try {
    const { email, linkedin, github } = req.body;
    const result = await userDB.updateOne(
      { mid: req.user.mid },
