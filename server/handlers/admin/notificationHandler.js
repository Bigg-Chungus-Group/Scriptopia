...
router.get("/receive", async (req, res) => {
  ...
  try {
    const notifications = await notificationDB.find({expiry: {$gte: dateToday}}).toArray();
...
router.post("/update", verifyToken, verifyAdminPrivilges, async (req, res) => {
  ...
  try {
    await notificationDB.updateOne({ _id: new ObjectId(notificationId) }, {$set: {body: notificationBody, expiry: new Date(notificationExpiry)}});
